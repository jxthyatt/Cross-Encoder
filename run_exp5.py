...
parser.add_argument('--z-dim-app', type=int, default=16,
                    help='size of 1D latent code for appearance (default: 32)')
parser.add_argument('--z-dim-gaze', type=int, default=32,
...
parser.add_argument('--num-training-epochs', type=float, default=450, metavar='N',
...
# data
parser.add_argument('--num-data-loaders', type=int, default=56, metavar='N',
...
parser.add_argument('--save-path', type=str, default='backup/exp5-resnet50',
                    help='path to save network parameters (default: .)')
# image saving
parser.add_argument('--save-image-samples', type=int, default=0,
                    help='save image samples or not, 0 for not saving, 1 for saving')
parser.add_argument('--image-path', type=str, default='images/exp1-resnet50',
                    help='path to save image samples')
parser.add_argument('--save-freq-images', type=int, default=999999999999,
...
from dataloaders import Columbia, UTMultiview, MPIIGaze, XGaze, Unite, FreeGaze
...
data_root = '/mnt/traffic/home/jxt/3ndPaper-Allworks/3-Datasets/train_dataset/'
#The training group.
#The validation group need to be excluded.

data_set = FreeGaze(data_root,
...
#scheduler = optim.lr_scheduler.StepLR(optimizer,step_size=30*len(data_loader),gamma = 0.5)
scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, 450*len(data_loader), eta_min=0, last_epoch=-1)
...
def execute_training_step(current_step):
    ...
    try:
        ...
        if num_elapsed_epochs >= 300 and num_elapsed_epochs % 10 == 0:
...
for current_step in range(initial_step, num_training_steps):
    ...
    # Save image samples
    if args.save_image_samples > 0 and current_step == last_training_step:
        network.eval()
        torch.cuda.empty_cache()
